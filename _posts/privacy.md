---

# Front Matter (YAML)

author: "The Alien Studio Team"
banner_alt: "Elder’s wise gaze, city lights blur, a lifetime etched in his eyes"
banner_height: "100vh"
banner_width: "100vw"
banner: "https://cloudcdn.pro/alienstudio/v1/collections/metropolis/metropolis-13.webp"
cdn: "https://cloudcdn.pro"
changefreq: "weekly"
charset: "UTF-8"
cname: ""
copyright: "© 2024 - 2026 Alien Studio Photography. All rights reserved."
date: "Jan 13, 2024"
description: "Learn how Alien Studio protects your privacy with zero tracking cookies, encrypted HTTPS connections, and strict data protection practices."
format-detection: "telephone=no"
form-id: ""
hreflang: "en"
icon: "https://cloudcdn.pro/alienstudio/v1/logos/alienstudio.svg"
id: "https://alienstudio.com/privacy/index.html"
image_alt: "Logo of Alien Studio: Redefining visual storytelling in the digital age."
image_height: "100vh"
image_width: "100vw"
image: "https://cloudcdn.pro/alienstudio/v1/logos/alienstudio.svg"
keywords: "privacy policy, zero tracking, data protection, privacy rights, Alien Studio privacy"
language: "en-GB"
layout: "link"
locale: "en_GB"
logo_alt: "Logo of Alien Studio: Redefining visual storytelling in the digital age."
logo_height: "44"
logo_width: "44"
logo: "https://cloudcdn.pro/alienstudio/v1/logos/alienstudio.svg"
menu: "active"
measurementID: "G-F37SP9MF2L"
name: "Privacy notice and cookies"
permalink: "https://alienstudio.com/privacy/"
rating: "general"
referrer: "no-referrer"
revisit-after: "7 days"
robots: "index, follow"
short_name: "alienstudio"
subtitle: "Your privacy is important to us."
tags: "privacy, personal data, tracking tools, Google Analytics, Microsoft Clarity, user behaviour analytics, mouse movements, clicks, scrolls, rights, contact"
theme-color: "255,255,255"
title: "Privacy Policy | Alien Studio"
url: "https://alienstudio.com/privacy/index.html"
viewport: "width=device-width, initial-scale=1, shrink-to-fit=no"

# RSS - The RSS feed front matter (YAML).
atom_link: "https://alienstudio.com/privacy/rss.xml"
category: "Technology"
docs: "https://validator.w3.org/feed/docs/rss2.html"
generator: "Static Site Generator (SSG) (version 0.0.26)"
item_description: RSS feed for the site
item_guid: "https://alienstudio.com/privacy/rss.xml"
item_link: "https://alienstudio.com/privacy/rss.xml"
item_pub_date: "2024-01-13T06:06:06+01:00"
item_title: "Alien Studio Privacy: How We Use Your Data"
last_build_date: "2024-01-13T06:06:06+01:00"
managing_editor: "contact@alienstudio.com"
pub_date: "2024-01-13T06:06:06+01:00"
ttl: "60"
type: "website"
webmaster: "contact@alienstudio.com"

# Apple - The Apple front matter (YAML).
apple_mobile_web_app_orientations: "portrait"
apple_touch_icon_sizes: "192x192"
apple-mobile-web-app-capable: "yes"
apple-mobile-web-app-status-bar-inset: "black"
apple-mobile-web-app-status-bar-style: "black-translucent"
apple-mobile-web-app-title: "Privacy Policy | Alien Studio"
apple-touch-fullscreen: "yes"

# MS Application - The MS Application front matter (YAML).

msapplication-navbutton-color: "255,255,255"

# Twitter Card - The Twitter Card front matter (YAML).

twitter_card: "summary"
twitter_creator: "@AlienStudioArt"
twitter_description: "This page informs you of our policies regarding the collection, use, and disclosure of personal data when you use our Website"
twitter_image: "https://cloudcdn.pro/alienstudio/v1/logos/alienstudio.svg"
twitter_image_alt: "Logo of Alien Studio: Redefining visual storytelling in the digital age."
twitter_site: "@AlienStudioArt"
twitter_title: "Alien Studio Privacy: How We Use Your Data"
twitter_url: "https://alienstudio.com/privacy/index.html"

# Humans.txt - The Humans.txt front matter (YAML).
author_website: "https://alienstudio.com/privacy/index.html"
author_twitter: "@AlienStudioArt"
author_location: "London, UK"
thanks: "Thanks for reading!"
site_last_updated: "2024-01-13"
site_standards: "HTML5, CSS3, RSS, Atom, JSON, XML, YAML, Markdown, TOML"
site_components: "Static Site Generator (SSG), Static Site Generator CLI, Static Site Generator Templates, Static Site Generator Themes, Kaishi SSG, Kaishi CLI, Kaishi Templates, Kaishi Themes"
site_software: "Static Site Generator, Rust"

---

## 1. Information We Do Not Collect

We believe fine art galleries should be enjoyed with total peace of mind. When browsing Alien Studio:
- We do not set behavioral advertising or marketing tracking cookies.
- We do not profile visitors or sell your browsing data to third parties.
- We do not perform canvas fingerprinting or cross-site tracking.

## 2. Information You Voluntarily Provide

When you submit an inquiry through our Contact Form (for gallery exhibitions, print acquisitions, or licensing), we collect:
- **Your Name:** To address you in our correspondence.
- **Your Email Address:** To reply to your inquiry.
- **Subject & Message Details:** To process and fulfill your request.

## 3. Third-Party Infrastructure & Privacy Standards

Alien Studio works exclusively with privacy-conscious service providers:
- **Formspree:** Contact form submissions are processed securely via Formspree's encrypted transmission endpoint under strict data processing agreements.
- **CloudCDN & Edge Distribution:** Assets are served via privacy-hardened edge caching with Subresource Integrity (SRI) verification to protect against tampering.
- **Etsy Storefront:** External links to our verified Etsy shop ([AlienStudioShop ⧉](https://www.etsy.com/shop/AlienStudioShop)) are governed by Etsy's independent privacy policy upon navigating to their domain.

## 4. Your Legal Rights (UK / EU GDPR & CCPA)

Depending on your jurisdiction, you possess the following statutory rights regarding your personal data:
- **Right of Access:** Request a copy of any personal communications we hold.
- **Right to Erasure ("Right to be Forgotten"):** Request immediate permanent deletion of your inquiry messages.
- **Right to Rectification:** Request correction of inaccurate information.
- **Right to Object & Restrict Processing:** Withdraw consent for communication at any time.

## 5. Contact Our Privacy Lead

If you have any questions about this Privacy Policy or wish to exercise your data rights, please contact our team at [contact@alienstudio.com](mailto:contact@alienstudio.com).
